    ...
    <div class="flex-1 max-w-xl">
        ...
        <div class="flex gap-1 mb-6 bg-gray-900 p-1 rounded-xl border border-gray-800">
            <button type="button" onclick="switchTab('general')" id="tab-general"
                class="flex-1 py-2.5 px-4 rounded-lg text-sm font-semibold transition-all bg-indigo-600 text-white">
                📢 Anuncio General
            </button>
            <button type="button" onclick="switchTab('fortnite')" id="tab-fortnite"
                class="flex-1 py-2.5 px-4 rounded-lg text-sm font-semibold transition-all text-gray-400 hover:text-white">
                🎮 Partida Fortnite
            </button>
        </div>
        ...
        <div id="form-general">
            <form action="{{ route('send') }}" method="POST" class="space-y-5" onchange="updatePreview()" oninput="updatePreview()">
                ...
                <button type="submit"
                    class="w-full py-3 px-6 bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800 text-white font-semibold rounded-xl transition-colors">
                    📤 Enviar Anuncio
                </button>
            </form>
        </div>
            ...
            <form action="{{ route('send') }}" method="POST" class="space-y-5" onchange="updatePreview()" oninput="updatePreview()">
                ...
                <div>
                    ...
                    <select name="mode" id="f-mode"
                        ...
                        <option value="no_build">⚡ Sin Construcción (Zero Build)</option>
                        <option value="ranked_build">🏆 Ranked Construcción</option>
                        <option value="ranked_no_build">🏆 Ranked Sin Construcción</option>
                    </select>
                </div>
    ...
    <div class="w-80 flex-shrink-0 sticky top-8">
        ...
        <div class="bg-gray-800 rounded-xl p-4 border border-gray-700">
            ...
            <div class="flex items-start gap-3">
                <div class="w-10 h-10 rounded-full bg-indigo-600 flex items-center justify-center text-white font-bold text-sm flex-shrink-0">🤖</div>
                <div class="flex-1 min-w-0">
                    ...
                    <div id="embed-preview" class="rounded-r-md overflow-hidden" style="border-left: 4px solid #5865F2;">
                        <div class="bg-gray-700/80 p-3 space-y-2">
                            <div id="preview-title" class="font-semibold text-white text-sm">Título del anuncio</div>
                            <div id="preview-body" class="text-gray-300 text-xs leading-relaxed">Tu mensaje aquí...</div>
                            <div id="preview-fields" class="space-y-2 pt-1 hidden">
                                <div class="flex gap-4">
                                    <div>
                                        <p class="text-xs font-semibold text-gray-400">🕹️ Modo</p>
                                        <p id="pf-mode" class="text-xs text-white">Construcción</p>
                                    </div>
                                    <div>
                                        <p class="text-xs font-semibold text-gray-400">🌐 Región</p>
                                        <p id="pf-region" class="text-xs text-white">Europa</p>
                                    </div>
                                </div>
                                <div>
                                    <p class="text-xs font-semibold text-gray-400">🔑 Contraseña</p>
